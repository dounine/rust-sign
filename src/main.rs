...
extern "C" {
    ...
    pub fn sign_ipa(ipa_path: *const std::os::raw::c_char, key_path: *const std::os::raw::c_char, mp_path: *const std::os::raw::c_char, dylib_file_path: *const std::os::raw::c_char, icon_path: *const std::os::raw::c_char, tmp_folder_path: *const std::os::raw::c_char, error: *mut std::os::raw::c_char) -> *const std::os::raw::c_uint;
}
...
fn main() {
    unsafe {
        ...
        let ipa_path = std::ffi::CString::new("/Users/lake/dounine/github/rust/rust-sign/ipa/video.ipa").unwrap();
        let key_path = std::ffi::CString::new("/Users/lake/dounine/github/rust/rust-sign/ipa/key.pem").unwrap();
        let mp_path = std::ffi::CString::new("/Users/lake/dounine/github/rust/rust-sign/ipa/lake_13_pm.mobileprovision").unwrap();
        let dylib_file_path = std::ffi::CString::new("/Users/lake/dounine/github/rust/rust-sign/ipa/d.dylib").unwrap();
        let icon_path = std::ffi::CString::new("/Users/lake/dounine/github/rust/rust-sign/ipa/icon.png").unwrap();
        let tmp_folder_path = std::ffi::CString::new("/Users/lake/dounine/github/rust/rust-sign/ipa/tmp").unwrap();

        let mut error_mut: [std::os::raw::c_char; 1024] = [0; 1024];
        // let c_error = get_hello(error_mut.as_mut_ptr());
        sign_ipa(ipa_path.as_ptr(), key_path.as_ptr(), mp_path.as_ptr(), dylib_file_path.as_ptr(), icon_path.as_ptr(), tmp_folder_path.as_ptr(), error_mut.as_mut_ptr());
